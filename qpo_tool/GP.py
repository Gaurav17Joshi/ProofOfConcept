...
def build_gp(kernel_params, mean_params, t, kernel_type = "QPO_plus_RN", mean_type = "gaussian"):
...
class GP:
    ...
    def __init__(self, Lc: Lightcurve, Model_type: tuple, Model_params: dict = None) -> None:
        ...
        self.maingp = GaussianProcess(self.kernel, Lc.time, mean = self.mean, diag=Model_params["diag"])

        # self.maingp = build_gp(kernel_params = Model_params, mean_params = Model_params, 
        #                        t = Lc.time, kernel_type = Model_params["kernel_type"], 
        #                        mean_type = Model_params["kernel_type"])
    ...
    def plot_originalgp(self, X_test = None, sample_no=1):
        """
        ...
        y_samp = self.maingp.sample(jax.random.PRNGKey(3), shape=(sample_no,))
...
def get_prior(kernel_type, mean_type, windowed = None, **kwargs):
    """
    ...
    def RNprior_model():
        ...
        t0 = yield Prior(tfpd.Uniform(kwargs["Times"][0]-0.1*kwargs["T"], kwargs["Times"][-1]+0.1*kwargs["T"]), name='t0')
...
def get_likelihood(kernel_type, mean_type, **kwargs):
    ...
    def RNlog_likelihood(arn, crn, A, t0, sig):
        ...
        gp = build_gp(rnlikelihood_params, mean_params, kwargs["Times"], kernel_type = "RN", mean_type = mean_type)
    ...
    def QPOlog_likelihood(arn, crn, aqpo, cqpo, freq,  A, t0, sig):
        ...
        mean_params = {"A": A, "t0": t0, "sig": sig,}
        print("hi")
        gp = build_gp(qpolikelihood_params, mean_params, kwargs["Times"], kernel_type = "QPO_plus_RN", mean_type = mean_type)
        print(gp)
        print(gp.kernel)
        print(len(gp.X))
        print(type(gp.X))
        print("log_likelihood", gp.log_probability(kwargs["counts"]))
        return gp.log_probability(kwargs["counts"])
...
class GPResult:
    ...
    def run_sampling(self):
        ...
        """
        self.prior_model = get_prior(self.prior_type[0], self.prior_type[1], **self.prior_parameters)
        self.likelihood = get_likelihood(self.prior_type[0], self.prior_type[1], **self.prior_parameters)
        print(self.prior_model)
        for obs in self.prior_model():
            print(obs)
        ...
        Termination_reason, State = self.Exact_ns(random.PRNGKey(42), term_cond=TerminationCondition(live_evidence_frac=1e-4))
